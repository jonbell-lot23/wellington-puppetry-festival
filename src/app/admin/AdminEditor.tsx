...
import { saveSiteContent, type SiteContent, type ContentVersion } from '../actions'
...
export default function AdminEditor({
  ...
  return (
    <div className="relative">
      ...
      <div className="fixed top-0 inset-x-0 z-50 bg-black/90 border-b border-yellow-400/30 px-6 py-3 flex items-center justify-between backdrop-blur">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-1">
            {tabBtn('edit', 'Edit')}
            {tabBtn('history', `History${initialVersions.length > 0 ? ` (${initialVersions.length})` : ''}`)}
          </div>
        </div>
      ...
      {tab === 'history' ? (
        <div className="min-h-screen pt-16 px-6 py-8 max-w-2xl mx-auto" style={{ backgroundColor: '#1a0a0a' }}>
          <h2 className="text-white font-bold text-lg mb-1">Version history</h2>
          <p className="text-white/40 text-xs mb-6">Each save is snapshotted. Click a version to see what changed, then revert if needed.</p>
          {migrationSQL ? (
            <div className="border border-yellow-400/30 bg-yellow-400/5 rounded-lg p-4 mb-6">
              <p className="text-yellow-300 font-bold text-sm mb-2">DB setup required</p>
              <p className="text-white/50 text-xs mb-3">Run this once in your <a href="https://supabase.com/dashboard" target="_blank" className="underline hover:text-white">Supabase SQL editor</a>:</p>
              <pre className="text-green-300 text-xs bg-black/40 rounded p-3 overflow-x-auto select-all">{migrationSQL}</pre>
            </div>
          ) : (
            <HistoryPanel versions={initialVersions} current={initialContent} />
          )}
        </div>
      ) : (
